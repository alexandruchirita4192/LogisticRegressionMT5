...
FEATURE_COLS = [
    ...
    "vol_10", "vol_20", "vol_ratio_10_20",
    "dist_sma_10", "dist_sma_20", "zscore_20",
    "atr_pct_14", "range_pct_1", "body_pct_1",
]


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train LogisticRegression classifier for MT5 and export to ONNX.")
    ...
    parser.add_argument("--output-dir", type=str, default="output_lr_classifier")
...
def make_classifier(c_value: float, max_iter: int, random_state: int) -> Pipeline:
...
def walk_forward_report(train_df: pd.DataFrame, label_quantile: float, prob_quantile: float, margin_quantile: float, n_splits: int, c_value: float, max_iter: int, random_state: int) -> Dict[str, float]:
    X = train_df[FEATURE_COLS].to_numpy(dtype=np.float32)
    tscv = TimeSeriesSplit(n_splits=n_splits)

    accepted_rates, directional_precisions = [], []
    mean_trade_returns, ternary_accs, bal_accs = [], [], []
    entry_thresholds, gap_thresholds = [], []
    ...
    for fold, (fit_idx, valid_idx) in enumerate(tscv.split(X), start=1):
        ...
        model = make_classifier(c_value=c_value, max_iter=max_iter, random_state=random_state + fold)
        model.fit(fit_df[FEATURE_COLS].to_numpy(dtype=np.float32), fit_df["target_class_enc"].to_numpy(dtype=np.int64))

        entry_prob_threshold, min_prob_gap = derive_decision_thresholds(model, fit_df, prob_quantile, margin_quantile)
        pred_df = classify_with_thresholds(model, valid_df, entry_prob_threshold, min_prob_gap)
...
def main() -> None:
    ...
    model = make_classifier(c_value=args.c, max_iter=args.max_iter, random_state=args.random_state)
    model.fit(train_lab[FEATURE_COLS].to_numpy(dtype=np.float32), train_lab["target_class_enc"].to_numpy(dtype=np.int64))

    entry_prob_threshold, min_prob_gap = derive_decision_thresholds(model, train_lab, args.prob_quantile, args.margin_quantile)
    ...
    onnx_path = output_dir / "ml_strategy_classifier_logistic_regression.onnx"
    ...
    (output_dir / "logistic_regression_metadata.json").write_text(json.dumps(metadata, indent=2), encoding="utf-8")

    run_txt = f"""MODEL: LogisticRegression multiclass classifier
SYMBOL: {args.symbol}
TIMEFRAME: {args.timeframe}
